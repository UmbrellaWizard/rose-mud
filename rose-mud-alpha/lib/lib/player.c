...
int Setup(){
    ...
    if( !GetClass() && !GetProperty("guesty_mcguestguest") )SetClass("explorer");
    else SetClass("guest");
    ...
    if(GetProperty("guesty_mcguestguest")){
        AddChannel( ({ "public" }) );
        this_object()->SetInvis(1);
	SetProperty("guesty_mcguestguest",0);
    }
    ...
    if(this_object()->GetTown() == "FirstAdmin"){
        ...
        this_object()->AddChannel( ({"admin", "error", "cre", "newbie", "gossip", "public", "ds", "ds_test", "lpuni", "death", "connections","intercre","dchat"}) );

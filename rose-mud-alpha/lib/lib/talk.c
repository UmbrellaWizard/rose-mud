 ...
 *    Version: @(#) talk.c 1.5@(#)
 *    Modified by Umbrella Wizard 06/24/15
 */
...
int eventTalkHist(string str, string type){
    ...
    TalkHist[type] += ({ clean_time(tz) + " " + str });
    return 1;
}
...
string array GetTalkHistTypes(){
    if(!TalkHist) TalkHist = ([]);
    if(!TalkHist["say"]) TalkHist["say"] = ({});
    if(!TalkHist["ooc"]) TalkHist["ooc"] = ({});
...
varargs mixed eventHearTalk(object who, object target, int cls, string verb,
    ...
    switch(cls){
        ...
        case TALK_LOCAL_OOC:
            tmp = "%^BOLD%^YELLOW%^<" + verb + "> " + who->GetName() + ": " + msg + "%^RESET%^";
            this_object()->eventPrint(tmp, MSG_CONV);
...
varargs mixed eventSpeak(object target, int cls, string msg, string lang){
    ...
    cols = GetScreen()[0];
    if( msg[<1] != '?' && msg[<1] != '!' && msg[<1] != '.' && cls != TALK_LOCAL_OOC)
        msg = capitalize(msg) + ".";

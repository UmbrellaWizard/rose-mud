...
{
    string *logelements = ({});
    if(!file_exists("/log/reports/lastseen")) write_file("/log/reports/lastseen", "");
    if (strsrch(read_file("/log/reports/lastseen"), GetCapName()) == -1)
    {
         write_file("/log/reports/lastseen", "\n" + GetCapName()+ " " +clean_time(this_player()->GetProperty("timezone")));
    }
         else
         {
             logelements = explode(read_file("/log/reports/lastseen"), "\n");
             rm("/log/reports/lastseen");
             foreach(string logelement in logelements)
             {
                 if(strsrch(logelement, GetCapName()) != -1)
                 {
                     logelement = "\n" + GetCapName()+ " " +clean_time(this_player()->GetProperty("timezone"));
                 }
                 else
                 ...
                 write_file("/log/reports/lastseen", logelement);
           }
        }
    return;
}

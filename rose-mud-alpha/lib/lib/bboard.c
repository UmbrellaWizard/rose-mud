...
void end_post(string subj, string mail){
    ...
    if( !mail && previous_object() != "/cmds/players/report")
        BBOARD_D->add_post(query_board_id(),
                this_player()->GetCapName(), subj, msg);
    if(base_name(this_object()) == "/cmds/players/report")
    {
        if(msg == "" || regexp(msg, "[A-Z]|[a-z]") != 1)
        {
             message("system", "You must provide a reason for your report!\nReport not sent, please try again.", this_player());
	     return;
        }
        write_file("/log/reports/reportlist", clean_time(this_player()->GetProperty("timezone")) + " " + this_player()->GetCapName() 
                   + " reported " + cap_string(subj) +".\nReason: " + msg + "\n");
	message("system", "Report sent.", this_player());
	return;
    }
    else
    {

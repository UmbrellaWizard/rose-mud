...
varargs mixed eventHearTalk(object who, object target, int cls, string verb,
    ...
    switch(cls){
        ...
        case TALK_LOCAL:
            ...
            if(query_privs(this_object()) != convert_name(who->GetName())) 
            {
		room = "/log/room";
                if(!directory_exists(room + base_name(this_object()) + "/"))
		{
                    string *elems = explode(base_name(this_object()), "/");
	            foreach(string subdir in elems)
                    {       
                	room = room + "/" + subdir;    
			if(!directory_exists(room))
                        {
                            mkdir(room);
                        }
		    }
                }
                else room = room + base_name(this_object());
		write_file(room + "/say", clean_time(tz) + " " + who->GetName() + " " + pluralize(verb) + " in " + capitalize(who->GetDefaultLanguage()) + ", \"%^BOLD%^CYAN%^" + msg + "%^RESET%^\"");
	        if(last(read_file(room + "/say"), 2) != "\n")  write_file(room + "/say", "\n");
	    }
		...
		write_file(room + "/ooc", clean_time(tz) + " %^BOLD%^YELLOW%^<OOC> " + who->GetName() + ": " + msg + "%^RESET%^");
                if(last(read_file(room + "/ooc"), 2) != "\n")  write_file(room + "/ooc", "\n");
	    }
            if(sizeof(obs)) obs -= ({ who });
            if(sizeof(obs))
                obs->eventHearTalk(who, target, cls, "OOC", msg, lang);
            obs = get_livings(this_object(),2);
            if(sizeof(obs)) obs -= ({ who });
            if(sizeof(obs))
                obs->eventHearTalk(who, target, cls, "OOC", msg, lang);

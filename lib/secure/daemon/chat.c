...
static void Setup(){
    ...
    mixed rchan3 = ({});
    remote_chans = ({});
    local_chans = ({"newbie","cre","gossip", "public", "admin","error", "intermud",
...
int cmdLast(string feep){
    ...
    {
        if(feep != "ooc") this_player()->eventPrint("You are not subscribed to that channel.", MSG_ERROR);
        return 1;
    }
...
varargs int eventAddLast(string feep, string str, string pchan, string pmsg, string pwho)
{
    ...
    if(pchan && pchan != "admin"){
        LogIt(clean_time(this_player()->GetProperty("timezone")) + " " + plainmsg + "\n", "/log/chan/"+Chan, Chan);
    }
    else {
...
varargs void eventSendChannel(string who, string ch, string msg, int emote,
    ...
    if( emote ) {
        ...
        tmp = sprintf(emotelayout, this_msg, capitalize(ch), "%^RESET%^", msg);
        ...
        if(Channels[ch]){
            ...
            if( member_array(ob, obs) != -1 ) {
                if( ob && !(ob->GetBlocked(ch)) ) {
                    int ignore;
                    tmp = sprintf(emotelayout, this_msg, capitalize(ch), "%^RESET%^", targmsg);
        ...
        //Put together the channel emote message
        msg = sprintf(chatlayout, who, chancolor, capitalize(ch), "%^RESET%^", pmsg);

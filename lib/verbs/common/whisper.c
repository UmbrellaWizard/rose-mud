...
static void create() {
    ...
    else SetErrorMessage("Whisper to whom what?");
...
mixed can_whisper_to_liv(object target) {
    ...
    else return "What is it you are trying to whisper?";
}

mixed can_whisper_in_wrd(string lang) {
    if(GUEST_CHECK == "guest") return "You can't do that."; 
    else return "Whisper to whom in " + lang + "?";
}

mixed can_whisper_in_wrd_to_liv(string wrd, object ob) {
    if(GUEST_CHECK == "guest") return "You can't do that."; 
    else return "What do you mean to whisper?";
}
...
mixed can_whisper_to_liv_str(object targ, string str) {
    ...
    if(GUEST_CHECK == "guest") return "You can't do that."; 
    else return can_whisper_to_liv_in_wrd_str(targ, lang, str);
}
...
mixed can_whisper_to_liv_in_wrd_str(object targ, string lang, string str) {
    if(GUEST_CHECK == "guest") return "You can't do that."; 
    else return this_player()->CanSpeak(targ, TALK_SEMI_PRIVATE, str, lang);
}

...
mixed can_shout() { 
    if(GUEST_CHECK == "guest") "You can't do that.";
    else return "Shout what?"; 
}

mixed can_shout_str(string str) {
    string lang;
    if( !str || GUEST_CHECK == "guest") return "You can't do that.";
...
mixed can_shout_in_wrd_str(string lang, string str) {
    if( !lang || !str || GUEST_CHECK == "guest") return "You can't do that.";

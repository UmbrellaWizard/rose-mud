...
mixed cmd(string args) {
    ...
    if(query_privs(environment(this_player())) != convert_name(this_player()->GetName()))
    {                 
         room = "/log/room";
         if(!directory_exists(room + base_name(environment(this_player())) + "/"))
	 {
              string *elems = explode(base_name(environment(this_player())), "/");
         ...
         else room = room + base_name(environment(this_player()));
         write_file(room + "/say", clean_time(tz) + " " + this_player()->GetName() + args);
